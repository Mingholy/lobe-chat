...
const VISION_MODEL = new Set(['qwen-vl-plus', 'qwen-vl-max']);
const DEFAULT_BASE_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1';

/* Use DashScope OpenAI compatible mode for now */
/* DashScope OpenAI [compatible mode](https://help.aliyun.com/zh/dashscope/developer-reference/tongyi-qianwen-vl-plus-api) currently does not support base64 image input for vision models e.g. qwen-vl-plus */
export class LobeQwenAI extends LobeOpenAICompatibleRuntime implements LobeRuntimeAI {
  ...
  private buildCompletionParamsByModel(payload: ChatStreamPayload) {
    ...
    const isVisionModel = VISION_MODEL.has(model);

    const params = {
      ...payload,
      messages: isVisionModel ? this.unifyVLMessages(messages) : messages,
  ...
  private unifyVLMessages(messages: OpenAIChatMessage[]) {
